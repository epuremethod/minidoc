...
/**
 * Compile one YAML fixture into a source-mapped Vitest suite. This module only
 * parses, validates and generates code — what a scenario's data means is the
 * business of the resolved steps module. Slated for extraction into
 * @epure/vitest, next to its Gherkin compiler.
 *
 * A fixture is a single YAML document: a feature title, a background, then
 * scenario examples:
 *
 * ```yaml
 * feature: <feature title>     # the describe title
 * background:
 *   given: a filesystem        # the step that runs every scenario
 * examples:
 *   - scenario: it should ...  # the test description
 *     source: ...              # everything else is data for the step
 *     target: ...
 * ```
 */
export type CompileOptions = {
  /** Steps module registering `given` handlers, or null when none was found. */
  stepsPath: string | null;
  /** Module exporting the `runScenario` step runtime. */
  runtimePath: string;
...
export function compile(path: string, options: CompileOptions): { code: string; map: RawSourceMap } {
  ...
  push(`import { runScenario } from ${JSON.stringify(options.runtimePath)};`, top);
